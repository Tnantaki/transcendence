...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.query_param = {k: v for (k, v) in [i.split(
        ...
        if self.room.number_of_player == self.room.size:
            await self.channel_layer.group_send(
                self.room_group_name, {
                    "type": "game.control", 'command': 'START', "sender": "SERVER", }
            )
            await self.channel_layer.group_send(
                self.room_group_name, {
                    "type": "game.assign", 'command': 'ASSIGN_PLAYER', "sender": "SERVER", }
            )
        ...
        if message["type"] == "game-control":
            await self.channel_layer.group_send(
                self.room_group_name, {
                    ...
                    "type": "game.control", 'command': message['command'], "player": message["player"], }
            )
    ...
    async def game_control(self, event):
        ...
        await self.send(text_data=json.dumps({"command": message, "player": player}))

    async def game_assign(self, event):
        message = event["command"]

        # Send message to WebSocket
        await self.send(text_data=json.dumps({"command": message, "player": self.player_number}))
